...
StringVal ToUpperCaseUtf8(FunctionContext* context, const StringVal& arg1) {
  ...
  char * aa = (char *) g_utf8_strup((const gchar*) original.c_str(), original.size());
  std::string upper(aa);

  // The modified string is stored in 'upper', which is destroyed when this function
  // ends. We need to make a string val and copy the contents.
  // NB: Only the version of the ctor that takes a context object allocates new memory.
  StringVal result(context, upper.size());
  memcpy(result.ptr, upper.c_str(), upper.size());
  return result;
}
...
StringVal ToLowerCaseUtf8(FunctionContext* context, const StringVal& arg1) {
  ...
  char * aa = (char *) g_utf8_strdown((const gchar*) original.c_str(), original.size());
  std::string lower(aa);

  // The modified string is stored in 'lower', which is destroyed when this function
  // ends. We need to make a string val and copy the contents.
  // NB: Only the version of the ctor that takes a context object allocates new memory.
  StringVal result(context, lower.size());
  memcpy(result.ptr, lower.c_str(), lower.size());
  return result;
}
